...
public class AccountDAOImpl implements AccountDAO {
    ...
    @Override
    public double sumTotalMoneyOfService() {
        Connection connection = DBConnection.getConnection();
        PreparedStatement statement = null;
        ResultSet resultSet = null;
        ...
        if (connection != null){
            try {
                statement = connection.prepareStatement("select sum(contract_total_money) from contract;");
                resultSet = statement.executeQuery();
                while (resultSet.next()){
                    total_money = resultSet.getDouble("sum(contract_total_money)");
                }
            } catch (SQLException e) {
                e.printStackTrace();
            } finally {
